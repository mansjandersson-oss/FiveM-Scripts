...
function resourceName() {
  return typeof GetParentResourceName === "function" ? GetParentResourceName() : "nb-chopshop";
}
  ...
  const rows = contract.vehicles.map((vehicle) => {
    ...
    return `
      <div class="vehicle-row">
        ...
        <div class="badge ${done ? "done" : ""}">${done ? "Klar" : `#${escapeHtml(vehicle.index)}`}</div>
      </div>
    `;
  }).join("");
    ...
    <main class="screen">
      <section class="panel" role="dialog" aria-label="${escapeHtml(contract.title)}">
        <header class="panel-header">
          ...
          <button class="icon-button" id="closeButton" type="button" title="Stäng" aria-label="Stäng">${icon("close")}</button>
        </header>
        ...
        <footer class="panel-footer">
          <button class="button secondary" id="refreshButton" type="button">${icon("refresh")}Uppdatera</button>
          <button class="button primary" id="turnInButton" type="button" ${contract.complete ? "" : "disabled"}>${icon("money")}Lämna in</button>
        </footer>
      </section>
    </main>
  `;
