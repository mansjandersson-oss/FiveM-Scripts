...
local function isCriminal(src, player)
    ...
    return hasConfiguredAccess(player)
end
...
local function updateContractItemMetadata(src, metadata)
    ...
    if not slot then
        if Config.Debug then
            print(('[chopshop] %s not found for source %s'):format(Config.Items.chop_contract, src))
        end
        return
    end
...
RegisterNetEvent('chopshop:server:GetContract', function()
    ...
    if existing and existing.vehicles and getRemainingContractVehicles(existing) > 0 then
        ...
        TriggerClientEvent('chopshop:client:ShowContract', src, getContractPayload(existing))
        return
    end
    ...
    TriggerClientEvent('chopshop:client:SetContractVehicles', src, getContractPayload(contracts[src]))
    TriggerClientEvent('chopshop:client:ShowContract', src, getContractPayload(contracts[src]))
...
RegisterNetEvent('chopshop:server:ViewContract', function()
    ...
    TriggerClientEvent('chopshop:client:ShowContract', src, getContractPayload(contract))
end)
...
RegisterNetEvent('chopshop:server:TurnInContract', function()
    ...
    TriggerClientEvent('chopshop:client:ClearContract', src)
...
RegisterNetEvent('chopshop:server:CheckContractVehicle', function(modelName)
    ...
    for _, vehicle in ipairs(contract.vehicles) do
        if vehicle.model:lower() == modelName:lower() and not contract.completed[vehicle.model] then
            TriggerClientEvent('chopshop:client:ContractVehicleDetected', src, vehicle.label)
            return
        end
    end
end)
...
RegisterNetEvent('chopshop:server:StripFrame', function(netId, modelName)
    ...
    for _, vehicle in ipairs(contract.vehicles) do
        if vehicle.model:lower() == modelName:lower() and not contract.completed[vehicle.model] then
            ...
            TriggerClientEvent('chopshop:client:SetContractVehicles', src, getContractPayload(contract))
