...
local function applyVehicleTarget(vehicle)
    ...
    if not isVehicleAllowedForCurrentJob(vehicle) then return end
        ...
        if shouldShowStripPartForVehicle(vehicle, part) and not isPartStripped(netId, part.name) then
            ...
            options[#options + 1] = {
                ...
                onSelect = function()
                    ...
                    TriggerServerEvent('chopshop:server:StripPart', netId, partName, partItem)
...
local function spawnNPC(data, options)
    ...
    if model == 0 or not IsModelValid(model) or not IsModelInCdimage(model) or not IsModelAPed(model) then
        if Config.Debug then print(('[chopshop] invalid NPC model: %s'):format(tostring(data.model))) end
        return nil
    end

    if not loadModel(model) then
        if Config.Debug then print(('[chopshop] failed to load NPC model: %s'):format(tostring(data.model))) end
        return nil
    end

    local ped = CreatePed(4, model, data.coords.x, data.coords.y, data.coords.z - 1.0, data.coords.w, true, true)
    if ped == 0 or not DoesEntityExist(ped) then
        if Config.Debug then print(('[chopshop] failed to spawn NPC: %s'):format(tostring(data.name or data.model))) end
...
RegisterNetEvent('chopshop:client:SetContractVehicles', function(contractData)
...
RegisterNetEvent('chopshop:client:ClearContract', function()
...
RegisterNUICallback('refresh', function(_, cb)
    TriggerServerEvent('chopshop:server:ViewContract')
    cb({ ok = true })
end)

RegisterNUICallback('turnIn', function(_, cb)
    TriggerServerEvent('chopshop:server:TurnInContract')
    cb({ ok = true })
end)
