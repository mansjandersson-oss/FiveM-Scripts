...
RegisterNetEvent('nb-carheist:client:startMission', function(data)
...
RegisterNetEvent('nb-carheist:client:policePing', function(coords)
                    ...
                    options = {
                        {
                            ...
                            icon = 'check',
                            onSelect = function()
                                TriggerServerEvent('nb-carheist:server:requestMission')
                            end
                        },
                        {
                            title = t('menu_cancel'),
        ...
        if state.decryptDone and inVeh then
            ...
            if dist < 12.0 then
                ...
                if IsControlJustReleased(0, 38) then
                    lib.hideTextUI()
                    TriggerServerEvent('nb-carheist:server:completeMission', state.missionPlate)
                end
            else
                lib.hideTextUI()
            end
...
CreateThread(function()
    while true do
        ...
        if speed > 3.0 then
            ...
            TriggerServerEvent('nb-carheist:server:sendPolicePing', { x = coords.x, y = coords.y, z = coords.z })
        end
...
RegisterNetEvent('nb-carheist:client:missionCompleted', function()
    notify(t('mission_complete'), 'success')
    resetMission()
end)

RegisterNetEvent('nb-carheist:client:notify', function(msg, nType)
    notify(msg, nType)
end)
