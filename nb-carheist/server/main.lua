...
local function notify(src, msg, nType)
    TriggerClientEvent('nb-carheist:client:notify', src, msg, nType or 'inform')
end
...
RegisterNetEvent('nb-carheist:server:requestMission', function()
    local src = source
    ...
    TriggerClientEvent('nb-carheist:client:startMission', src, {
        model = model,
        spawn = spawn,
        plate = plate
    })
...
RegisterNetEvent('nb-carheist:server:sendPolicePing', function(coords)
    local src = source
    if not activeMissions[src] then return end

    for _, player in pairs(QBCore.Functions.GetQBPlayers()) do
        if player.PlayerData.job and player.PlayerData.job.name == 'police' and player.PlayerData.job.onduty then
            TriggerClientEvent('nb-carheist:client:policePing', player.PlayerData.source, coords)
        end
    end
end)

RegisterNetEvent('nb-carheist:server:completeMission', function(plate)
    local src = source
    ...
    Player.Functions.AddMoney(Config.Reward.type, Config.Reward.amount, 'nb-carheist')

    activeMissions[src] = nil
    missionCooldown[src] = os.time() + 300

    TriggerClientEvent('nb-carheist:client:missionCompleted', src)
end)
